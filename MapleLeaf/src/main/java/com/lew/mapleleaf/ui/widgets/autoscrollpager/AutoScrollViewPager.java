...
public class AutoScrollViewPager extends AutoScrollBase implements ViewPager.OnPageChangeListener {
    ...
    // 默认滑动时间间隔
    // 默认指示器Margin Bottom
    private final int DEFAULT_BOTTOM_MARGIN = Util.dip2px(getContext(), 10);
    // 默认指示器间隔
    private final int DEFAULT_INDICATOR_SPACE = Util.dip2px(getContext(), 5);

    private PageControlBase.Adapter mIndicatorAdapter;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (null != attrs) {
            ...
            mIndictorVisibleInSingle = array.getBoolean(R.styleable.AutoScrollViewPager_indicatorVisibleInSingle, false);
            mIndictorVisible = array.getBoolean(R.styleable.AutoScrollViewPager_indicatorVisible, true);
            mIndictorBottomMargin = array.getDimension(R.styleable.AutoScrollViewPager_indicatorBottomMargin, DEFAULT_BOTTOM_MARGIN);
            mIndictorSpace = array.getDimension(R.styleable.AutoScrollViewPager_indicatorSpace, DEFAULT_INDICATOR_SPACE);
            scrollFactor = array.getFloat(R.styleable.AutoScrollViewPager_scrollFactor, 1.0f);
            mCycleScrollingEnable = array.getBoolean(R.styleable.AutoScrollViewPager_cycleScrollingEnable, true);

            array.recycle();
        }

        mViewPager = new EnhancedViewPager(context, attrs);
        mViewPager.setId(R.id.scrollView);
        LayoutParams vp_lp = new LayoutParams(LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
        ...
        LayoutParams pc_lp = new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
        ...
        mPageControl.setIndicatorSpace(mIndictorSpace);
        ...
        mScrollTask = new Runnable() {
            @Override
            public void run() {
                if (null != mAdapter && mAdapter.getCount() > 1 && mAdapter.count() >= mViewPager.getCurrentItem() + 1) {
                    mViewPager.setCurrentItem(mViewPager.getCurrentItem() + 1);
                }
            }
        };
        // 设置一个默认指示器监听器
        setOnIndicatorClickListener(new OnIndicatorClickListener() {
            @Override
            public void onIndicatorClick(View itemView, int position) {
    ...
    @Override
    public void setIndicatorBottomMargin(int bottomMargin) {
        if (null != mPageControl) {
            LayoutParams lp = (LayoutParams) mPageControl.containerView().getLayoutParams();
            lp.bottomMargin = Util.dip2px(getContext(), bottomMargin);
    ...
    public void setIndicatorSpace(int space) {
        mIndictorSpace = Util.dip2px(getContext(), space);
        if (null != mPageControl) {
    ...
    @Override
    public void setIndicatorVisibleInSingle(boolean visibleInSingle) {
        mIndictorVisibleInSingle = visibleInSingle;
    }

    public void setIndicatorAdapter(PageControlBase.Adapter indicatorAdapter) {
        if (null == indicatorAdapter) {
            indicatorAdapter = new DefaultIndicatorAdapter(getContext());
        }
        mIndicatorAdapter = indicatorAdapter;
        if (null != mPageControl) {
            mPageControl.setAdapter(indicatorAdapter);
        }
    }
    ...
    public void updateIndicatorStatus() {
        if (null != mPageControl) {
            if (mPageControl.mTotalPage <= 1) {
                ...
                if (mPageControl.mCurrPage > mPageControl.mTotalPage)
                    mPageControl.setCurrPage(currPage());
            }
        }
    }
    ...
    public void setAdapter(AutoScrollPagerAdapter adapter) {
        if (null != adapter) {
            ...
            setIndicatorAdapter(mIndicatorAdapter);
        }
    }

    @Override
    public void setOnIndicatorClickListener(final OnIndicatorClickListener onIndictorClickListener) {
        if (null != onIndictorClickListener && null != mPageControl) {
            mPageControl.setOnItemClickListener(new PageControlBase.OnItemClickListener() {
                ...
                public void onItemClick(View itemView, int position) {
                    onIndictorClickListener.onIndicatorClick(itemView, position);
                }
            });
        }
    }
    ...
    public void onPageSelected(int position) {
        if (null != mIndicatorAdapter && (null != mPageControl && mPageControl.isVisible())) {
            if (null != mAdapter) {
                mIndicatorAdapter.setCurrPosition(position % mAdapter.getCount());
            }
        }
        if (null != onPageChangeListener && mAdapter != null) {
